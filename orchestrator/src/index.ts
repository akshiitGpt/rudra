...
import { RudraEventBus } from "./events";
import { LocalPipelineRunner } from "./pipeline";
import { LinearCliTrackerClient } from "./tracker";
...
import { RudraOrchestrator } from "./orchestrator";
...
async function main(): Promise<void> {
  ...
  const logsRoot = process.argv[3]
    ...
    : process.env.RUDRA_LOGS_ROOT
      ? path.resolve(process.env.RUDRA_LOGS_ROOT)
      : path.join(os.homedir(), ".rudra-runs");
  const eventBus = new RudraEventBus();
  ...
  const tracker = new LinearCliTrackerClient(workflow.config.tracker);
  ...
  const apiKey = requireConfiguredApiKey(process.env.RUDRA_API_KEY);
  const orchestrator = new RudraOrchestrator(
    tracker,
    workflowStore,
    pipelineRunner,
  ...
  const apiServer = createApiServer({
    ...
    logsRoot,
    apiKey,
    corsOrigin: process.env.RUDRA_CORS_ORIGIN,
    reviewLoop,
  });
  const apiPort = Number.parseInt(process.env.RUDRA_API_PORT ?? "3847", 10);
  const apiHost = process.env.RUDRA_API_HOST ?? "0.0.0.0";
