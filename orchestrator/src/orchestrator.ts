...
function dispatchPlanHookEnv(dispatchPlan: DispatchPlan): Record<string, string> {
  return {
    RUDRA_BASE_BRANCH: dispatchPlan.baseBranch,
    RUDRA_TARGET_BRANCH: dispatchPlan.targetBranch,
    RUDRA_MERGE_STRATEGY: dispatchPlan.mergeStrategy,
  };
}
...
export class RudraOrchestrator {
  ...
  constructor(
    ...
    private readonly eventBus?: RudraEventBus,
    ...
    return async () => {
      if (!cachedBranchInfo) {
        cachedBranchInfo = (async () => {
          try {
            const github = new GhCliClient(githubConfig);
  ...
  private refreshRedispatchBarriers(candidateIssues: Issue[]): void {
    ...
    // to a different state. That keeps Rudra from thrashing on the same active issue.
  ...
  private async buildDispatchPlan(
    ...
    if (triage.action === "request-clarification") {
      if (this.tracker.commentOnIssue) {
        try {
          await this.tracker.commentOnIssue(issue.id, triage.comment ?? "Rudra needs more detail before it can start this work.");
        } catch (error) {
          console.error(JSON.stringify({
    ...
    try {
      ...
      if (result.status === "wait_human") {
          ...
          if (escalationConfig.comment && this.tracker.commentOnIssue) {
            try {
              await this.tracker.commentOnIssue(
                latestIssue.id,
                formatEscalationComment(result.error ?? "Rudra needs human review."),
              );
            } catch (error) {
              console.error(JSON.stringify({
        ...
        this.eventBus?.emit({
          ...
          ...issueEventFields(latestIssue),
          completedNodes: result.completedNodes,
          reason: result.error ?? "Rudra needs human review.",
        });
        return;
      }
