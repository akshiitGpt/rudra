...
const DEFAULT_CONFIG = {
  ...
  workspace: {
    root: "/tmp/rudra-workspaces",
  },
  artifacts: {
    root: "~/rudra-artifacts/issues",
    workspaceDir: ".rudra",
  },
  hooks: {
    timeoutMs: 60_000,
...
function parseGitHubConfig(github: Record<string, unknown>): GitHubConfig | null {
  ...
  return {
    ...
    apiKey,
    webhookSecret,
    revisionLabel: String(github.revision_label ?? "rudra-revision").trim() || "rudra-revision",
    revisionCommand: String(github.revision_command ?? "/rudra revise").trim() || "/rudra revise",
