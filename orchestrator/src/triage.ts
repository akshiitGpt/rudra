...
function normalizedDecision(parsed: Record<string, unknown>, workflow: WorkflowDefinition, issue: Issue): TriageDecision {
  ...
  if (action === "request-clarification") {
    const comment = String(parsed.comment ?? "").trim()
      || "Rudra needs more detail before it can start this work. Please clarify the expected change, affected area, and any branch or rollout constraints.";
    return {
      action,
      comment,
...
export async function triageIssue(opts: {
  ...
  const skillPath = path.join(opts.skillsRoot ?? builtInSkillsRoot(), "rudra-triage", "SKILL.md");
  ...
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "rudra-triage-"));
  try {
