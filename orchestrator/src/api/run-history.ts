...
type LoggedEventShape = RudraEvent & {
  _sequence?: number;
};

export interface LoggedRudraEvent {
  sequence: number;
  event: RudraEvent;
}

type RunLifecycleEvent =
  | Extract<RudraEvent, { type: "issue:dispatched" }>
  | Extract<RudraEvent, { type: "issue:completed" }>
  | Extract<RudraEvent, { type: "issue:escalated" }>
  | Extract<RudraEvent, { type: "issue:failed" }>
  | Extract<RudraEvent, { type: "issue:cancelled" }>
  | Extract<RudraEvent, { type: "issue:retry:scheduled" }>
  | Extract<RudraEvent, { type: "pipeline:stage:start" }>
  | Extract<RudraEvent, { type: "pipeline:stage:complete" }>;
...
function parseLoggedEventLine(line: string, fallbackSequence: number): LoggedRudraEvent | null {
  try {
