...
export function createApiServer(opts: {
  eventBus: RudraEventBus;
  orchestrator: RudraOrchestrator;
  ...
  app.get("/events", async (request: FastifyRequest<{
    ...
    const formatEvent = (event: RudraEvent, sequence?: number) => {
      const payload = { ...(event as RudraEvent & { _sequence?: number }) };
      delete payload._sequence;
      return `${sequence !== undefined ? `id: ${sequence}\n` : ""}data: ${JSON.stringify(payload)}\n\n`;
    };
    const listener = (event: RudraEvent) => {
      const liveSequence = (event as RudraEvent & { _sequence?: number })._sequence;
