...
const MANAGED_SKILL_PREFIX = "rudra-";
const MANAGED_SKILL_TARGETS = [
  ".codex/skills",
  ".claude/skills",
] as const;
const EXCLUDE_BLOCK_START = "# BEGIN Rudra managed skills";
const EXCLUDE_BLOCK_END = "# END Rudra managed skills";
...
async function listManagedSkills(skillsRoot: string): Promise<string[]> {
  ...
  if (skillNames.length === 0) {
    throw new Error(`no managed Rudra skills found in ${skillsRoot}`);
  }
