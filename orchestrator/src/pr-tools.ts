...
/**
 * Built-in PR commands using `gh` CLI.
 *
 * `rudra publish-pr` and `rudra update-pr` are tool stages that create or
 * update pull requests via the `gh` CLI wrapper.
 */
...
function parseArgs(command: string): { subcommand: BuiltInPrCommand; args: Map<string, string | true> } | null {
  ...
  if (tokens[0] !== "rudra" || (tokens[1] !== "publish-pr" && tokens[1] !== "update-pr")) {
    return null;
  }
  ...
  return { subcommand: tokens[1], args };
}
...
function findExistingPrByBranch(pullRequests: Array<{ number: number; headRefName: string | null }>, branch: string): number | null {
  const match = pullRequests.find((pr) => String(pr.headRefName ?? "").trim() === branch);
...
async function persistWorkspacePr(workspacePath: string, pr: PullRequestMetadata): Promise<void> {
  const filePath = path.join(workspacePath, ".rudra", "pr.json");
...
async function readStoredPrNumber(workspacePath: string): Promise<number | null> {
  try {
    const payload = JSON.parse(await readFile(path.join(workspacePath, ".rudra", "pr.json"), "utf8")) as Record<string, unknown>;
...
async function publishPr(opts: {
  ...
  const github = new GhCliClient(opts.githubConfig);
...
export async function executeBuiltInRudraTool(opts: {
  command: string;
