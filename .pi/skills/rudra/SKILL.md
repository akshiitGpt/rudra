---
name: rudra
description: Use when asked about Rudra — the background coding agent. Covers how to assign work, create workflows/skills/agents, monitor runs, and troubleshoot.
---

# Rudra — Background Coding Agent

Rudra picks up Linear issues, routes them through multi-step pipelines, and ships PRs. It polls Linear every 30s for issues assigned to it.

## Giving Rudra work

1. **Create a Linear issue** with a clear title and description (the description IS the prompt — Rudra agents read it verbatim)
2. **Assign to Rudra** — the Linear user configured as the assignee in WORKFLOW.md
3. **Set state to Todo** (Rudra polls `Todo` and `In Progress`)
4. **Optionally add a label** to route to a specific workflow (see Workflow Routing below)
5. Rudra picks it up within ~30s, creates a workspace, and runs the pipeline
...
### Artifacts
Each run produces artifacts in `.rudra/run/` (workspace). Key artifacts: `plan.md`, `implementation-summary.md`, `code-review.md`, `pr-body.md`.
...
2. **Create the DOT pipeline** file:
   ...
   digraph MyPipeline {
     ...
     step_one [type="agent", label="Step One", agent="my-writer", artifact_path=".rudra/run/output.md"]
     step_two [type="agent", label="Step Two", agent="my-reviewer", artifact_path=".rudra/run/output.md"]

     start -> step_one -> step_two -> exit
   }
   ```

3. **Add skills** (optional) to `orchestrator/skills/rudra-my-skill/SKILL.md`
...
When Rudra creates a PR and it receives review feedback:
1. A reviewer submits "Changes Requested" or comments the configured revision command
2. GitHub webhook hits Rudra
3. Rudra adds the revision label to the Linear issue and moves it to the revision state
...
|---------|---------------|
| `rudra state` | Active runs, retries, barriers |
| `rudra runs` | Recent run history with status |
| `rudra run_detail` (issue + attempt) | Full detail of a specific run |
| `rudra config` | Current configuration |
| `rudra agents` | All configured agents |
| `rudra skills` | All skills |
| `rudra workflows` | All workflows with routing |
| `rudra workflow_detail` (name) | Full detail of a workflow |
...
- **Redispatch barrier**: After success or terminal failure, Rudra won't re-run the same issue until its Linear state changes
